...
class RMOT_Dataset(Dataset):
    ...
    def _parse_data(self):
        labels = json.load(open(os.path.join(self.opt["DATA_ROOT"], 'Refer-KITTI_labels.json'), 'r'))
        data = multi_dim_dict(2, list)
        target_expressions = defaultdict(list)
        expression_dir = os.path.join(self.opt["DATA_ROOT"],'expression')
...
class Track_Dataset(Dataset):
    ...
    def _parse_data(self):
        ...
        for video in VIDEOS[self.mode]:
            # load tracks
            tracks_1 = np.loadtxt(join(self.opt["TRACK_ROOT"],self.opt["TRACK_NAME"], video, 'car', 'predict.txt'), delimiter=',')
            if len(tracks_1.shape) == 2:
                ...
                max_obj_id = 0
            tracks_2 = np.loadtxt(join(self.opt["TRACK_ROOT"],self.opt["TRACK_NAME"], video, 'pedestrian', 'predict.txt'), delimiter=',')

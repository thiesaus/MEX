...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, d_model,seq_length,n_head=4,dropout=0.,device=None):
        super().__init__()
        self.d_model = d_model
        self.n_head = n_head
        ...
        self.c_attn = nn.Linear(d_model, 3 * d_model, bias=self.bias,device=device)
        # output projection
        self.c_proj = nn.Linear(d_model, d_model, bias=self.bias,device=device)
        # regularization
...
class MLP(nn.Module):
    ...
    def __init__(self, d_model,dropout=0.,device=None,bias=True):
        super().__init__()
        self.d_model = d_model
        self.c_fc    = nn.Linear(d_model, 4 * d_model,device=device, bias=bias)
        self.gelu    = nn.GELU()
        self.c_proj  = nn.Linear(4 * d_model, d_model,device=device, bias=bias)
        self.dropout = nn.Dropout(dropout)
